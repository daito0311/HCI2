...
public class EjemploC extends PApplet {
	...
	public void draw() {
		...
		case 3:
			...
			if (comC.cicloJuego == 3) {
				if (comC.scrnSecs >= 5) {
					...
					if (comC.eleccion == 0 && comC.eleccionOtroJugador == 2) {
						comC.posMono -= 1;
			...
			if (comC.cicloJuego == 4) {
				if (comC.scrnSecs >= 5) {
					...
					System.out.println("FUERZA TOTAL" + fuerzaacumulada);
		...
		case 8:
			...
			fill(255);
			textSize(50);
			text("EMPATE", width / 2 - 100, height / 2);
	...
	public void mouseClicked() {
		...
		if (comC.cicloJuego == 0 && comC.turno == 3) {
			comC.enviar("Empezar Juego");
		}
